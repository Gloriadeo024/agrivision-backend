...
cropSchema.methods.analyzeFeedbackSentiment = function (feedbackData) {
  const sentimentScore = feedbackData.length * 0.75;
  return sentimentScore > 5 ? 'Positive' : 'Neutral';
};

// AI-powered water efficiency calculation (placeholder)
cropSchema.methods.calculateWaterEfficiency = function (irrigationData) {
  const totalWaterUsed = irrigationData.reduce((sum, record) => sum + record.amount, 0);
  return this.currentYield / totalWaterUsed || 0;
};
